...
// Added functions for Modularity
  void printHex(uint8_t *data, uint8_t length);
  void calcChecksum(byte *checksumPayload, byte payloadSize) ;
  void sendUBX(byte *UBXmsg, byte msgLength, HardwareSerial gpsSerial);
  byte getUBX_ACK(byte *msgID,HardwareSerial gpsSerial);
  void setBaud(byte baudSetting, HardwareSerial gpsSerial);
  void configureUblox(byte *settingsArrayPointer, HardwareSerial gpsSerial);
  void displayInfo();
private:
